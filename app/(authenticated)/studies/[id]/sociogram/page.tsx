    ...
    <div className={`flex bg-background text-foreground ${fullscreen ? 'fixed inset-0 z-50' : 'h-screen'}`}>
      ...
      <div className="w-64 flex-shrink-0 bg-card border-r border-border flex flex-col overflow-hidden">
        ...
        <div className="px-4 pt-4 pb-3 border-b border-border shrink-0"
          style={{ background: 'linear-gradient(135deg, var(--primary, #6D28D9), color-mix(in srgb, var(--primary, #6D28D9) 65%, black 35%))' }}>
            ...
            {analytics && (
              <div className="px-3 py-2.5 border-b border-border">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {[
                    { l:'Nodes',      v:analytics.n,           c:'#6D28D9' },
                    { l:'Edges',       v:analytics.e,           c:'#0EA5E9' },
                    { l:'Density',     v:`${analytics.density}%`, c:'#F97316' },
                    { l:'Reciprocity', v:`${analytics.reciprocity}%`, c:'#10B981' },
                    { l:'Clustering',  v:analytics.clustering,  c:'#EC4899' },
                    { l:'Modularity',  v:analytics.modularity,  c:'#EAB308' },
                    { l:'Communities', v:analytics.communities, c:'#8B5CF6' },
                    { l:'Components',  v:analytics.components,  c:'#14B8A6' },
                    { l:'Isolates',    v:analytics.isolates,    c:analytics.isolates>0?'#DC2626':'#6B6B80' },
                    { l:'Avg in',      v:analytics.avgIn,       c:'#0EA5E9' },
                    { l:'Avg out',     v:analytics.avgOut,      c:'#F97316' },
                  ].map(s => (
          ...
          <div className="absolute z-50 pointer-events-none"
            ...
            <div className="bg-card border border-border rounded-2xl shadow-2xl p-4 w-52"
              ...
              <div className="grid grid-cols-2 gap-1.5 mb-3">
                {[
                  { l:'Received', v:vizData.indegree[tipNode.id], c:'#10B981' },
                  { l:'Sent', v:vizData.edges.filter(e=>e[0]===tipNode.id).length, c:'#0EA5E9' },
                ].map(s => (
              ...
              <div className="space-y-1">
                ...
                {[
                  { l:'Betweenness', v:vizData.metrics.betweenness[tipNode.id]?.toFixed(3)??"—", c:'#6D28D9' },
                  { l:'Closeness',   v:vizData.metrics.closeness[tipNode.id]?.toFixed(3)??"—",   c:'#F97316' },
                  { l:'Eigenvector', v:vizData.metrics.eigenvector[tipNode.id]?.toFixed(3)??"—", c:'#EC4899' },
                ].map(s => (
