...
export function AddIatDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      // 1. Create iat_instruments record
      const iatInsert: Record<string, any> = {
        study_id: studyId,
        created_by: user.id,
        title: label.trim(),
        description: notes.trim() || null,
        iat_type: DEATH_SUICIDE_IAT.key,
        // Target categories
        category_a: DEATH_SUICIDE_IAT.targetA,
        category_b: DEATH_SUICIDE_IAT.targetB,
        // Attribute categories
        attribute_positive: DEATH_SUICIDE_IAT.attributePositive,
        attribute_negative: DEATH_SUICIDE_IAT.attributeNegative,
        // Stimuli stored as JSONB
        stimuli: {
          self: DEATH_SUICIDE_IAT.selfWords,
          other: DEATH_SUICIDE_IAT.otherWords,
          death: DEATH_SUICIDE_IAT.deathWords,
          life: DEATH_SUICIDE_IAT.lifeWords,
        },
        is_active: true,
        estimated_duration_minutes: DEATH_SUICIDE_IAT.estimatedMinutes,
        citation: DEATH_SUICIDE_IAT.citation,
        clinical_note: DEATH_SUICIDE_IAT.clinicalNote,
      }
